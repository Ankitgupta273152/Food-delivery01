...
export default function PendingOrders() {
  ...
  const updateLocalShopOrder = (orderId, shopOrderId, updatedShopOrder) => {
    if (!ownerPendingOrders) return;
    const updated = ownerPendingOrders.map((order) => {
      if (order._id === orderId && order.shopOrder?._id === shopOrderId) {
        return {
          ...order,
  ...
  const updateStatus = async (orderId, shopOrderId, status) => {
    try {
      const res = await axios.post(
        `${serverUrl}/api/order/update-order-status/${orderId}/${shopOrderId}`,
        { status },
      ...
      if (res.data.shopOrder) {
        updateLocalShopOrder(orderId, shopOrderId, res.data.shopOrder);
      }
  ...
  // Real-time socket listener for new orders and updates
  useEffect(() => {
    if (!socket) return;

    const handleNewOrder = (data) => {
      if (data?.order && data?.ownerId) {
        // Dispatch with computed array - avoid stale closure
        dispatch(setOwnerPendingOrders([data.order, ...ownerPendingOrders]));
        console.log("✅ New order received in real-time:", data.order._id);
      }
    };

    socket.on("orders:new", handleNewOrder);

    // Listen for status updates (e.g., delivered)
    const handleStatusUpdate = (data) => {
      if (!data) return;
      const orderId = data.orderId;
      const updatedShopOrder = data.shopOrder;
      if (!orderId || !updatedShopOrder) return;
      // Update local state: find matching entry and replace shopOrder
      const updated = ownerPendingOrders.map((order) => {
        if (order._id === orderId && order.shopOrder?._id === updatedShopOrder._id) {
          return { ...order, shopOrder: updatedShopOrder };
        }
        return order;
      });
      dispatch(setOwnerPendingOrders(updated));
      console.log('✅ Owner pending order updated via socket', orderId);
    };
  ...
  <div className="flex flex-col items-center gap-6">
    {ownerPendingOrders?.map((order) => {
      ...
      const shopOrderId = order.shopOrder?._id;
...
<div className="flex justify-between items-center mt-auto pt-3 border-t border-gray-100">
  ...
  {order.shopOrder?.status !== "delivered" && (
    <select
      ...
      onChange={(e) =>
        updateStatus(orderId, shopOrderId, e.target.value)
      }
      defaultValue=""
    >
