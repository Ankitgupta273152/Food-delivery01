...
function Nav() {
    const { city, userData, cartItems,pendingOrdersCount, searchItems } = useSelector(state => state.user);
    ...
    return (
        <div className="w-full h-[80px] flex items-center justify-between md:justify-center gap-[30px] px-[20px] fixed top-0 z-[9999] bg-[#fff9f6] overflow-visible">
            ...
            {showSearch && userData?.role === "user" && (
                <div className="w-[90%] h-[70px] bg-white shadow-xl rounded-lg items-center gap-[20px] z-[9999] flex fixed left-[5%] top-[80px]">
                    ...
                    <div className="w-[80%] flex items-center gap-[10px] relative">
                        ...
                        {input && (
                            <RxCross2 
                                className="w-[20px] h-[20px] text-[#ff4d2d] cursor-pointer absolute right-2 hover:opacity-70" 
                                onClick={clearSearch}
                            />
                        )}
                    </div>
            ...
            <div className="flex items-center gap-[20px]">
                ...
                {userData?.role === "user" && (
                    !showSearch ? (
                        <IoIosSearch className="w-[25px] h-[25px] text-[#ff4d2d] md:hidden cursor-pointer" onClick={() => setShowSearch(true)} />
                    ) : (
                        <RxCross2 className="w-[25px] h-[25px] text-[#ff4d2d] md:hidden cursor-pointer" onClick={toggleSearch} />
                    )
                )}
