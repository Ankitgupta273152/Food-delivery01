...
export const placeOrder = async (req, res) => {
  try {
    ...
    // 🔹 Emit socket event to notify shop owners about new order (COD)
    // Populate order data before emitting
    let populatedOrder = await Order.findById(newOrder._id)
      .populate("user", "fullName name email mobile")
    ...
    if (io && populatedOrder) {
      // Notify each shop owner about new COD order
      populatedOrder.shopOrders.forEach(shopOrder => {
        io.to(`user_${shopOrder.owner._id.toString()}`).emit("orders:new", {
          ownerId: shopOrder.owner._id.toString(),
          order: {
      ...
      // Also notify the customer that order was created
      io.to(`user_${populatedOrder.user._id}`).emit("orders:new", {
        order: {
          _id: populatedOrder._id,
          user: populatedOrder.user,
          address: populatedOrder.address,
          paymentMethod: populatedOrder.paymentMethod,
          createdAt: populatedOrder.createdAt,
          shopOrders: populatedOrder.shopOrders,
        },
      });
    }
...
// 🔹 Emit to the specific user's room, not globally
const io = req.app.get("io");
if (io) {
  // Send update to the customer who placed the order
  io.to(`user_${order.user._id}`).emit("orders:statusUpdated", {
...
export const updateDeliveryBoyLocation = async (req, res) => {
  try {
    ...
    // 3) Emit live update to the specific order room
    const io = req.app.get("io");
    if (io && orderId) {
      // Emit to the specific order room (both user and owner are listening)
      io.to(`order_${orderId}`).emit("delivery:locationUpdate", {
...
export const verifyDeliveryOtp = async (req, res) => {
  try {
    ...
    // Populate fresh order and emit status update to owner and customer
    ...
    const io = req.app.get('io');
    if (io && populated) {
      const updatedShopOrder = populated.shopOrders.id(shopOrder._id);
      // Notify customer
      io.to(`user_${populated.user._id}`).emit('orders:statusUpdated', {
        orderId: populated._id,
        shopOrder: updatedShopOrder,
        status: 'delivered'
      });

      // Notify shop owner for this shopOrder (if owner exists)
      if (updatedShopOrder && updatedShopOrder.shop && updatedShopOrder.shop.owner) {
        io.to(`user_${updatedShopOrder.shop.owner.toString()}`).emit('orders:statusUpdated', {
          orderId: populated._id,
          shopOrder: updatedShopOrder,
          status: 'delivered'
        });
